...
import AdminTable from './AdminTable'; // Import AdminTable
...
const AdminForm = () => {
  ...
  const handleEdit = (index, field, value) => {
    try {
      ...
      const studentToUpdate = updatedStudentDetails[index];

      if (studentToUpdate) {
        studentToUpdate[field] = value;
        setStudentDetails(updatedStudentDetails);
      } else {
        console.error(`Student at index ${index} not found.`);
      }
    } catch (error) {
...
const handleSave = async (index) => {
  try {
    ...
    await axios.post('/updateStudentDetails', {
      regNo,
      studentName,
      marksTheory,
      marksProject,
  ...
  return (
    <div>
      ...
      <div className="student-details-container">
       ...
       onEdit={handleEdit}  // Ensure this matches the function name in your AdminForm
       onSave={handleSave}
     ...
     />
      ...
      <div>
        ...
        <button
          ...
          disabled={!selectedBatchCode || fetchingData}
        >
          {fetchingData ? (
            <div className="loading-spinner">Loading...</div>
          ) : (
            'Send Notification to Certificate Department'
          )}
        </button>
      </div>
